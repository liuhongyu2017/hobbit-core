...
@ConditionalOnProperty(value = HobbitLogLevel.REQ_LOG_PROPS_PREFIX
...
public class RequestLogAspect {
   ...
   */
  @Around(
      "execution(!static org.hobbit.core.tool.api.R *(..)) && " +
          "(@within(org.springframework.stereotype.Controller) || " +
          "@within(org.springframework.web.bind.annotation.RestController))"
  )
  public Object aroundApi(ProceedingJoinPoint point) throws Throwable {
    HobbitLogLevel level = properties.getLevel();
    // 不打印日志，直接返回
    if (HobbitLogLevel.NONE == level) {
    ...
    try {
      ...
      // 打印返回结构体
      if (HobbitLogLevel.BODY.lte(level)) {
   ...
   * @param beforeReqArgs beforeReqArgs
   */
  public void logIngHeaders(HttpServletRequest request, HobbitLogLevel level,
      StringBuilder beforeReqLog, List<Object> beforeReqArgs) {
    // 打印请求头
    if (HobbitLogLevel.HEADERS.lte(level)) {
